...
class GSStrategy(BaseStrategy):
    def __init__(self, config, model: MixtureOfGaussians) -> None:
        ...
        self.clone_shading_normal_grad_threshold = (
            self.conf.strategy.densify.clone_shading_normal_grad_threshold
        )
        self.split_shading_normal_grad_threshold = (
            self.conf.strategy.densify.split_shading_normal_grad_threshold
        )
    ...
    def get_strategy_parameters(self) -> dict:
        ...
        params["densify_shading_normal_grad_norm_accum"] = (
            self.densify_shading_normal_grad_norm_accum,
        )
        params["densify_shading_normal_grad_norm_denom"] = (
            self.densify_shading_normal_grad_norm_denom,
        )
    ...
    def update_gradient_buffer(self, sensor_position: torch.Tensor) -> None:
        ...
        if params_position_grad is not None:
            ...
            distance_to_camera = (self.model.positions[mask] - sensor_position).norm(
                dim=1, keepdim=True
            )

            self.densify_grad_norm_accum[mask] += (
                torch.norm(
                    params_position_grad[mask] * distance_to_camera,
                    dim=-1,
                    keepdim=True,
                )
                / 2
    ...
    def split_gaussians(
        ...
        shading_normal_mask = (
            padded_shading_normal_grad >= self.split_shading_normal_grad_threshold
        )
        ...
        if self.conf.strategy.print_stats:
            ...
            n_split_by_normal = int(
                torch.logical_and(shading_normal_mask, size_mask).sum().item()
            )
            n_split_by_normal_only = torch.logical_and(
                shading_normal_mask, torch.logical_not(pos_mask)
            )
            n_split_by_normal_only = int(
                torch.logical_and(n_split_by_normal_only, size_mask).sum().item()
            )
            normal_pct = n_split_by_normal / n_split * 100 if n_split > 0 else 0.0
            normal_only_pct = (
                n_split_by_normal_only / n_split * 100 if n_split > 0 else 0.0
            )
            logger.info(
        ...
        def update_param_fn(name: str, param: torch.Tensor) -> torch.Tensor:
            ...
            p_new = torch.nn.Parameter(
                torch.cat([param[~mask], p_split]), requires_grad=param.requires_grad
            )
    ...
    def clone_gaussians(
        ...
        shading_normal_mask = (
            densify_shading_normal_grad_norm >= self.clone_shading_normal_grad_threshold
        )
        ...
        if self.conf.strategy.print_stats:
            ...
            n_clone_by_normal = int(
                torch.logical_and(shading_normal_mask, size_mask).sum().item()
            )
            n_clone_by_normal_only = torch.logical_and(
                shading_normal_mask, torch.logical_not(pos_mask)
            )
            n_clone_by_normal_only = int(
                torch.logical_and(n_clone_by_normal_only, size_mask).sum().item()
            )
            normal_pct = n_clone_by_normal / n_clone * 100 if n_clone > 0 else 0.0
            normal_only_pct = (
                n_clone_by_normal_only / n_clone * 100 if n_clone > 0 else 0.0
            )
            logger.info(
        ...
        def update_optimizer_fn(key: str, v: torch.Tensor) -> torch.Tensor:
            return torch.cat(
                [v, torch.zeros((int(mask.sum()), *v.shape[1:]), device=v.device)]
            )
    ...
    def prune_densification_buffers(self, valid_mask: torch.Tensor) -> None:
        ...
        self.densify_shading_normal_grad_norm_accum = (
            self.densify_shading_normal_grad_norm_accum[valid_mask]
        )
        self.densify_shading_normal_grad_norm_denom = (
            self.densify_shading_normal_grad_norm_denom[valid_mask]
        )
    ...
    def decay_albedo(self):
        ...
        def update_param_fn(name: str, param: torch.Tensor) -> torch.Tensor:
            ...
            desaturated_albedo = albedo.lerp(gray, weight.unsqueeze(-1)).clamp(
                1.0e-6, 1.0 - 1.0e-6
            )
            desaturated_albedo = self.model.material_albedo_activation_inv(
                desaturated_albedo
            )

            return torch.nn.Parameter(
                desaturated_albedo, requires_grad=param.requires_grad
            )

        self._update_param_with_optimizer(
            update_param_fn, None, names=["material_albedo"]
        )
