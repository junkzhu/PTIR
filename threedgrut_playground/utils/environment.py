...
class Environment:
    ...
    def __init__(
        self, folder: Optional[str] = None, device: Optional[torch.device] = None
    ):
        ...
        if folder is not None:
            self.available_envmaps += [
                f for f in os.listdir(folder) if f.lower().endswith(".hdr")
            ]
    ...
    def _load_hdr(self, envmap_name: Optional[str] = None) -> Optional[torch.Tensor]:
        ...
        if envmap_name not in self.available_envmaps:
            raise ValueError(
                f"Environment map {self.folder}{os.path.sep}{envmap_name} not found."
            )
    ...
    def set_env(self, env_name: Optional[str] = None) -> None:
        if env_name == "Model-Background":
            self._hdr_data = None
            self.envmap = torch.zeros(
                [512, 512, 4], dtype=torch.float32, device=self.device
            )
        elif env_name == "Black":
            self._hdr_data = None
            self.envmap = torch.zeros(
    ...
    def tonemap(self, hdr):
        ...
        if self.tonemapper == "Filmic":
            ...
            log_average_luminance = torch.exp(
                torch.mean(torch.log(epsilon + luminance))
            )
            ...
            raise ValueError(
                f"Invalid tonemapper. Must be one of {self.TONEMAPPER_OPTIONS}"
            )
