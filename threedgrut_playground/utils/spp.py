...
class SPP:
    def __init__(self, mode: str = "msaa", spp=4, batch_size=1, device="cuda"):
        ...
        assert mode == "msaa" and spp in (1, 2, 4, 8, 16), (
            "MSAA supports only power of 2 spp within [2,16]"
        )
        ...
        if mode == "msaa":
            self.msaa = StratifiedRayJitter(
                fixed_pattern=True, num_samples=spp, device=device
            )
    ...
    def __call__(self, img_h, img_w):
        ...
        if self.mode == "none":
            ...
            pixel_x = (
                torch.arange(img_w, device=self.device)
                .unsqueeze(0)
                .expand(img_h, img_w)
            )
            pixel_y = (
                torch.arange(img_h, device=self.device)
                .unsqueeze(1)
                .expand(img_h, img_w)
            )
